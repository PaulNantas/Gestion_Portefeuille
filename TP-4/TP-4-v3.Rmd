...
print(R.i2)
```
Avec les résltats obtenus, on peut raisonnablement penser que l'actif RUA est susceptible de recevoir un poids important dans le portefeuille optimal MV. L'actif GSPC possédant sensiblement les mêmes caractéristiques que l'actif RUA mais on peu moins rentable jouera sûrement un rôle moins important dans notres portefeuille.
...
for (i in 1:length(sigma.i)){
  ...
  wi2 = c(wi2 ,( mu[i] - unlist(R_i.t[i])[1:9]%*%mu[-i])/ (unlist(R_i.t[i])[10]**2 ) )
...
Finalement, on observe que GREXP est l'actif qui possède le poids le plus important dans notre portefeuille car possédant notamment un risque mesuré comparativement aux autres actifs. L'actif RUA possède bien un poids important car avec un rendement élevé.

Enfin, l'actif BG05.L possède bien comme prévu un poids négligé dans le portefeuille. 

Néanmoins on peut être un peu surpris par le rôle plutôt important joué par les actifs GLD et GDAXI étant donné leur rendement obtenu par régression pas meilleur que les autres actifs et leur risque plutôt important.
